...
// 标签数据
        ...
        function initDatePicker() {
            ...
            document.getElementById('dps-last-7-days').addEventListener('click', function() {
                ...
                // 更新显示
                trigger.querySelector('span').textContent = `${formatDisplayDate(formatDate(sevenDaysAgo))} 至 ${formatDisplayDate(formatDate(today))}`;
            });
            ...
            document.getElementById('dps-last-30-days').addEventListener('click', function() {
                ...
                // 更新显示
                trigger.querySelector('span').textContent = `${formatDisplayDate(formatDate(thirtyDaysAgo))} 至 ${formatDisplayDate(formatDate(today))}`;
            });
        ...
        const submitToPushBtn = document.getElementById('submit-to-push');
        ...
        // 提交到推送功能
        if (submitToPushBtn) {
            submitToPushBtn.addEventListener('click', function() {
                ...
                .then(data => {
                    if (data.success) {
                        showToast(`成功提交 ${data.updated_count} 条数据到推送`);
                        // 刷新当前页面数据
                        currentPage = 1;
                        performSearch(true);
                        // 取消全选
                        ...
                        // 触发自定义事件通知推送数据已更新
                        const event = new CustomEvent('pushedDataUpdated');
        ...
        function performSearch(resetPage = true) {
            ...
            // 显示加载状态
            if (currentPage === 1) {
                dataBody.innerHTML = '<tr><td colspan="9" class="text-center"><i class="fas fa-spinner fa-spin"></i> 加载中...</td></tr>';
            } else {
                ...
                if (data.results && data.results.length > 0) {
                    ...
                    data.results.forEach(item => {
                        ...
                        row.innerHTML = `
                            ...
                            <td>${formatDate(item.updated_time)}</td>
                        `;
