...
def get_bokeyuan_description_and_tag(informations):
    ...
    articles_tag = []
    # a = 1
    possible_classes = ["blogpost-body cnblogs-markdown", "postBody"]
    headers_list = {
    ...
    for info in informations:
        ...
        try:
            ...
            for class_name in possible_classes:
                soup_text = soup.find(class_=class_name)
                if soup_text is not None:
                    content = soup_text.get_text()
                    HotProjects.objects.filter(name=info['name']).update(content=content)
                    # 调用API
                    headers = {
                        'X-Source': 'openapi',
                        ...
                        "messages": [
                            {
                                ...
                                "content": [
                                    {
                                        "type": "text",
                                        "text": content[:5000],  # 限制文本长度，避免超出API限制
                                    }
                                ]
                            }
                        ]
            ...
            if soup_text is None:
                print(f"未在链接 {link} 中找到已记录的类名，请在possible_classes中添加类名")
...
def update_bokeyuan_descriptions(sql_links, articles_tag, articles_descriptions):
...
def save_github_readme(request):
    ...
    for repo_name in github_repo_name:
        ...
        try:
            ...
            """start"""
            saveCount = HotProjects.objects.filter(name=repo_name['name']).update(content=content, description=tag[1], tag=tag[0],
                                                                      updated_time=datetime.now())
            """end"""
            if saveCount > 0:
                print(f"成功更新 ID 为 {repo_name['name']} 的记录的 description 和 tag")
            else:
                print(f"ID 为 {repo_name['name']} 的记录不存在")
